...
class A2CAgent:
    ...
    def __init__(self, env: gym.Env, args=None, is_test=False):
        ...
        # 獎勵縮放和限制參數
        self.reward_scaling = getattr(args, 'reward_scaling', 0.1)  # 獎勵縮放係數
        self.reward_clamp_min = getattr(args, 'reward_clamp_min', -1.0)  # 獎勵最小值
        self.reward_clamp_max = getattr(args, 'reward_clamp_max', 1.0)  # 獎勵最大值
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.float64, bool]:
        ...
        if not self.is_test:
            ...
            clamped_reward = np.clip(scaled_reward, self.reward_clamp_min, self.reward_clamp_max)
            self.transition.extend([next_state, clamped_reward, done])
            return next_state, clamped_reward, done
        else:
            # 測試模式：使用原始獎勵，不進行縮放或限制
            if not self.is_test_mode:  # 只在訓練後的測試中記錄轉換
    ...
    def test(self, video_folder=None, checkpoint_path=None, num_episodes=1, render=True):
        """Test the agent."""
        self.is_test = True
        ...
        # 顯示測試資訊
        print(f"===== 測試開始 - 不進行獎勵縮放，使用原始獎勵值 =====")
        
        tmp_env = self.env
        if video_folder and render:
            # 如果沒有提供視頻文件夾，創建一個
        ...
        if video_folder and render:
            ...
            print(f"Test Summary: Mean Score = {np.mean(scores):.2f} ± {np.std(scores):.2f}")
    ...
    def save_config(self, args):
        ...
        config = {
            ...
            'reward_clamp_min': self.reward_clamp_min,
            'reward_clamp_max': self.reward_clamp_max,
        }
...
if __name__ == "__main__":
    ...
    # 獎勵相關參數
    parser.add_argument("--reward-scaling", type=float, default=0.1, help="獎勵縮放係數")
    parser.add_argument("--reward-clamp-min", type=float, default=-1.0, help="獎勵最小值")
    parser.add_argument("--reward-clamp-max", type=float, default=1.0, help="獎勵最大值")
